...
def filter_minimum_date(year_range:int,reviews_df):
	max_date = reviews_df['date'][0] 
	min_year = (max_date.year - year_range)
	print(type(min_year))

	for i in range(0, len(reviews_df)):
		if reviews_df['date'][i].year < min_year:
...
def extractComments(Count:int,score:int = None):
	...
	reviews_df = filter_minimum_date(2,reviews_df)
	for i in range(0, len(reviews_df)):
